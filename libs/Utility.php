...
class Utility {
    ...
    */
    ...
    /*
	public function yamlUrlCre($urlArr,$uri){
		$A = array();
    	#print"<pre>";var_dump($dir);exit;
    		if ($uri != "/") {
    		...
    		/*
    		if(in_array($uri,$urlArr)){
    			$kuzu = explode("/", $uri);
	    		foreach($kuzu as $key=>$val2){
	            	if($val2 != ""){
	            		$A[$n] = $val2;//配列
	            		$n = $n+1;
	            	}
	...
	public function yamlUrlCre($urlArrRow,$uri){
		$urlArr = array();
		$uriRev = ltrim($uri, '/');
		foreach($urlArrRow as $key=>$val){
			#print"<pre>";var_dump($val);exit;
    		if($key == $uriRev or $uriRev == ""){
    			$kuzu = explode("/", $uriRev);
	    		foreach($kuzu as $key2=>$val2){
	            		$urlArr[$key2] = $val2;//配列
	            }
		    	#print"<pre>";var_dump($urlArr);exit;
				return $urlArr;
